...
function updateSpreadsheetId(obj) {
  const spreadsheetId = obj.value;
...
function updateSigninStatus(isSignedIn) {
  if (isSignedIn) {
    ...
    authorize.style.display = 'inline-block';
...
customElements.define('plan-box', class extends HTMLElement {
  constructor() {
    super();
    const template = document.getElementById('plan-box').content.cloneNode(true);
...
function openDB(callback) {
  ...
  req.onupgradeneeded = e => {
    ...
    const index = db.createObjectStore('index', { keyPath:'level' });
...
function getIndex(db) {
  return new Promise((resolve, reject) => {
    ...
    tx.oncomplete = e => resolve(index);
    tx.onerror = e => reject(e);
    req.onsuccess = e => { index = e.target.result; };
  });
}
